...
export default class SelectionBox {
	...
	rectMouseDown(evt) {
		...
		this.emit("onBeforeMove", tempCache.rectStartPoint);
	}
	mouseMove(evt) {
	...
	mouseUp(evt) {
		if (tempCache.rectStartPoint) {
			...
			if (width < this.config.sizeThreshold && height < this.config.sizeThreshold) {
				...
				this.emit("onSelection", this.getBoxSelectionTargets());
			}
	...
	emit(eventName, params) {
		typeof this.config[eventName] === "function" && this.config[eventName](params);
	}
	dispose() {
